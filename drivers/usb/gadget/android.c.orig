...
static bool is_not_support_bam2bam_ipa;
	...
	if (!rmnet_initialized) {
		...
		while (b) {
			...
			if (ctrl_name && data_name) {
				if (tb)
					tname = strsep(&tb, ",");
				if(is_not_support_bam2bam_ipa) {
					if (!strncasecmp("BAM2BAM_IPA", data_name, XPORT_STR_LEN)) {
						err = frmnet_init_port(ctrl_name, "bam", tname);
					}
					else {
						err = frmnet_init_port(ctrl_name, data_name, tname);
					}
				}
				else {
					err = frmnet_init_port(ctrl_name, data_name, tname);
...
#ifdef CONFIG_HUAWEI_USB
	fsg_mod_data.luns = 1;//luns is 1 for init mass storage
...
static int android_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		...
		pdata->is_not_support_ipa = of_property_read_bool(pdev->dev.of_node,
				"qcom,no-support-ipa");
		is_not_support_bam2bam_ipa = pdata->is_not_support_ipa;
	} else {
